...
class ServerClient extends EventEmitter {
  constructor(server, deviceID, port, host) {
    super();
...
export default class Server extends EventEmitter {
  constructor(port) {
    ...
    this.socket.on('message', (msg, rinfo) => {
      ...
      if (topic == 'hi') {
        let client = new ServerClient(this, deviceID, rinfo.port, rinfo.address);
        this._clients[deviceID] = client;
        this.emit('client', client);
      } else {
        let client = this._clients[deviceID];
